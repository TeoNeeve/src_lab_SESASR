...
def landmark_sm_simpy_five_state():
    """
    Defines the landmark measurement model and its Jacobian Ht.
    ...
    """
    # Symbols for robot pose and landmark position
    x, y, theta, mx, my = symbols("x y theta m_x m_y")
    ...
    # Convert Sympy expressions for h(x, m) into a NumPy-evaluable function
    eval_hx = squeeze_sympy_out(sympy.lambdify((x, y, theta, mx, my), hx, "numpy"))

    # Jacobian Ht = dh/dx, with respect to the robot state [x, y, theta]
    Ht = hx.jacobian(Matrix([x, y, theta]))
    eval_Ht = squeeze_sympy_out(sympy.lambdify((x, y, theta, mx, my), Ht, "numpy"))
